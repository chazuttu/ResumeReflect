...
.chip {
    ...
    background: linear-gradient(135deg, rgba(192, 160, 128, 0.15), rgba(212, 175, 126, 0.1));
...
.chip:hover {
    background: linear-gradient(135deg, rgba(192, 160, 128, 0.25), rgba(212, 175, 126, 0.2));
    border-color: var(--accent-light);
}
...
.plan-price {
    ...
    color: var(--accent-light);
    margin: 1rem 0;
    background: linear-gradient(135deg, var(--accent-light), var(--silver));
...
if resume_method == "Upload PDF/DOCX":
    ...
    pasted = st.text_area("Paste your resume", height=180, placeholder="Paste here...", label_visibility="collapsed")
    if pasted:
...
jd_raw = st.text_area("Paste full job description", height=180, placeholder="Paste the complete job description here...", label_visibility="collapsed")
if jd_raw:
...
st.markdown('**Select Your Job Market:**')
...
if st.session_state.step >= 2 and st.session_state.ai_data:
    ...
    keywords = []
    found = data.get('ats_keywords_found', 0)
    for i in range(min(found, 15)):
        keywords.append(f'<span class="chip">Keyword {i+1}</span>')
    st.markdown(' '.join(keywords), unsafe_allow_html=True)
    
    st.markdown('**Missing Keywords:**')
    missing_keywords = []
    missing = data.get('ats_keywords_missing', 0)
    for i in range(min(missing, 10)):
        missing_keywords.append(f'<span class="chip">Missing Keyword {i+1}</span>')
    st.markdown(' '.join(missing_keywords), unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    with col1:
        import io
        resume_bytes = build_resume(data)
        if st.download_button("⬇ Download Tailored Resume", data=resume_bytes, file_name="resume_tailored.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True):
            log_user_action("resume_downloaded", email=st.session_state.get("email",""))
    
    with col2:
        analysis_bytes = build_analysis(data)
        if st.download_button("⬇ Download ATS Report", data=analysis_bytes, file_name="ats_report.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True):
