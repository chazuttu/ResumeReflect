...
if st.session_state.step >= 2 and st.session_state.ai_data:
    ...
    <div class="card" style="border-color:#00e5a044;margin-top:1rem">
        ...
        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:.6rem">
            <span style="background:#00e5a022;color:#00e5a0;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">$199 / 6 months</span>
    ...
    <div class="pricing-grid" style="grid-template-columns: repeat(2, 1fr);">
        ...
        <div class="plan hot" style="border-color: #ff7744;">
    ...
    col_b, col_p, col_m, col_y = st.columns(4)
    with col_b:
    ...
    with col_p:
        if st.button("Pay $10 →", key="pay_pro", use_container_width=True):
            ...
            st.success("🚀 Payments launching soon! We've noted your interest.")
    with col_m:
        if st.button("Pay $20 →", key="pay_monthly", use_container_width=True):
            log_payment_interest("Premium $20", st.session_state.email or "")
            st.success("🚀 Payments launching soon! We've noted your interest.")
    with col_y:
    ...
    with c2:
        plan = st.selectbox("Plan", ["Basic $5", "Pro $10", "Premium $20", "Premium Yearly $149"], label_visibility="collapsed")
