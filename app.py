...
# ─────────────────────────────────────────────
# REDDISH BG WITH GOLDEN YELLOW TEXT & BUTTONS
# ─────────────────────────────────────────────
st.markdown("""
<style>
...
.badge {
    ...
    background: linear-gradient(135deg,#d4a76a33,#b8860b33);
    border: 1px solid #d4a76a66;
    border-radius: 100px; padding: 5px 16px;
    font-size: 11px; letter-spacing: 2px; text-transform: uppercase;
    color: #ffd700; margin-bottom: 1.2rem;
}
.hero h1 {
...
.hero p { color: #d4a76a; font-size: 1rem; max-width: 480px; margin: 0 auto 2rem; line-height: 1.7; }

.card {
    background: var(--card); border: 1px solid #d4a76a;
    border-radius: var(--radius); padding: 1.5rem; margin-bottom: 1rem;
    transition: all 0.3s ease-in-out; box-shadow: 0 2px 12px rgba(212,167,106,0.1);
}
.card:hover { 
    border-color: #ffd700;
...
.card-title {
    ...
    color: #d4a76a; margin-bottom: 1rem; display: flex; align-items: center; gap: 8px;
}
.step-num {
...
.stFileUploader > div {
    background: var(--surface) !important; border: 2px dashed #d4a76a !important;
    border-radius: 12px !important; color: #d4a76a !important;
}
.stFileUploader > div:hover { border-color: #ffd700 !important; }
.stFileUploader button {
    background: linear-gradient(135deg,#d4a76a,#b8860b) !important;
    color: white !important; border: none !important;
    border-radius: 8px !important; font-weight: 700 !important;
}
.stFileUploader label { color: #d4a76a !important; font-weight: 600 !important; }
.stTextArea textarea {
    background: var(--surface) !important; border: 1px solid #d4a76a !important;
    border-radius: 12px !important; color: #1a1a1a !important;
    font-family: 'DM Sans', sans-serif !important; font-size: 14px !important;
}
.stTextArea textarea:focus { border-color: #ffd700 !important; box-shadow: 0 0 0 2px #d4a76a44 !important; }
.stTextInput input {
    background: var(--surface) !important; border: 1px solid #d4a76a !important;
    border-radius: 10px !important; color: #1a1a1a !important;
}
.stTextInput input:focus { border-color: #ffd700 !important; }
.stTextInput label { color: #d4a76a !important; font-weight: 600 !important; }
.stTextArea label { color: #d4a76a !important; font-weight: 600 !important; }

.stButton > button {
    background: linear-gradient(135deg,#d4a76a,#b8860b) !important;
    color: white !important; border: none !important; border-radius: 12px !important;
    padding: 14px 24px !important; font-family: 'Syne', sans-serif !important;
...
.stButton > button:hover { 
    ...
    box-shadow: 0 8px 25px rgba(212,167,106,0.4) !important;
    background: linear-gradient(135deg,#b8860b,#d4a76a) !important;
}
...
.stDownloadButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 8px 25px rgba(212,167,106,0.4) !important; }

.pricing-grid { display: grid; grid-template-columns: 1fr; gap: 1rem; margin: 1.2rem 0; }
@media (min-width: 768px) {
    .pricing-grid { grid-template-columns: repeat(3,1fr); }
}
.plan {
    background: #ffffff; border: 2px solid #d4a76a;
    border-radius: var(--radius); padding: 1.4rem; text-align: center;
    transition: all .2s; position: relative; box-shadow: 0 2px 12px rgba(212,167,106,0.15);
}
.plan:hover { border-color: #ffd700; transform: translateY(-3px); box-shadow: 0 10px 28px rgba(212,167,106,0.25); }
.plan.hot { border-color: #ffd700; background: #fffef0; }
.hot-badge {
...
.plan-name { font-family:'Syne',sans-serif; font-weight:700; font-size:.85rem; color: #d4a76a; text-transform:uppercase; letter-spacing:1.5px; margin-bottom:.4rem; }
.plan-price { font-family:'Syne',sans-serif; font-weight:800; font-size:1.8rem; color:#d4a76a; }
.plan-period { font-size:11px; color:#1a1a1a; margin-bottom:.8rem; }
.plan-feat { list-style:none; padding:0; margin:0 0 1rem; text-align:left; }
.plan-feat li { font-size:12px; color: #1a1a1a; padding:3px 0; display:flex; align-items:center; gap:6px; font-weight: 500; }
.plan-feat li::before { content:"✓"; color:#d4a76a; font-weight:700; }
.pay-btn {
    display:block; background:linear-gradient(135deg,#d4a76a,#b8860b);
...
.pay-btn:hover { box-shadow:0 5px 18px rgba(212,167,106,0.4); transform:translateY(-1px); }

/* NO REFUND POLICY STYLING - NEW */
.refund-policy {
    background: #a63a3a;
...
.score-box {
    flex:1; background:#ffffff; border:1px solid #d4a76a;
    border-radius:var(--radius); padding:1.2rem; text-align:center;
    box-shadow: 0 2px 12px rgba(212,167,106,0.1);
}
.score-val { font-family:'Syne',sans-serif; font-weight:800; font-size:2.2rem; color: #d4a76a; }
.score-before { color: #d4a76a; }
.score-after  { background:linear-gradient(135deg,#d4a76a,#b8860b); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
.score-label  { font-size:11px; color:#1a1a1a; margin-top:.3rem; text-transform:uppercase; letter-spacing:1px; }
.bar-wrap { background:var(--surface); border-radius:100px; height:8px; margin:.4rem 0; overflow:hidden; }
.bar { height:100%; border-radius:100px; background:linear-gradient(90deg,#d4a76a,#b8860b); }

.chip { display:inline-block; background:rgba(212,167,106,0.15); border:1px solid #d4a76a; color:#d4a76a; border-radius:100px; padding:4px 12px; font-size:12px; margin:3px; font-weight:500; }
.divider { height:1px; background:linear-gradient(90deg,transparent,#d4a76a,transparent); margin:2rem 0; }
.unlock-box { background: #a63a3a; border: 1px solid #d4a76a; border-radius: var(--radius); padding: 1.5rem; margin-top: 1rem; }

.stRadio > div { gap: 10px !important; }
.stRadio label { background:#ffffff !important; border:1px solid #d4a76a !important; border-radius:9px !important; padding:9px 14px !important; cursor:pointer !important; transition:all .15s !important; color: #1a1a1a !important; }
.stRadio label:hover { border-color:#ffd700 !important; }
label, .stRadio p { color: #d4a76a !important; font-size:13px !important; font-weight: 600 !important; }
.stSelectbox > div > div { background:#ffffff !important; border:1px solid #d4a76a !important; border-radius:10px !important; color:#1a1a1a !important; }
.stSelectbox label { color: #d4a76a !important; font-weight: 600 !important; }
.stTabs [data-baseweb="tab-list"] { background:var(--surface) !important; border-radius:10px !important; padding:3px !important; border:1px solid #d4a76a !important; }
.stTabs [data-baseweb="tab"] { background:transparent !important; color:#d4a76a !important; border-radius:7px !important; }
.stTabs [aria-selected="true"] { background:linear-gradient(135deg,#d4a76a,#b8860b) !important; color:white !important; }

/* ── Review Section ── */
.review-section-title {
    font-family:'Syne',sans-serif; font-weight:800; font-size:1.8rem;
    text-align:center; margin-bottom:.5rem; color:#ffd700;
}
.review-section-sub {
    text-align:center; color:#d4a76a; font-size:14px; margin-bottom:2rem;
}
.reviews-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:1rem; margin:2rem 0 2rem; }
.review-card {
    background:#ffffff; border:1px solid #d4a76a;
    border-radius:12px; padding:1.5rem;
    box-shadow:0 2px 12px rgba(212,167,106,0.1); transition: all .2s;
}
.review-card:hover { border-color:#ffd700; box-shadow:0 8px 24px rgba(212,167,106,0.2); transform:translateY(-2px); }
.review-stars { color:#ffc107; font-size:16px; margin-bottom:.8rem; letter-spacing:2px; }
.review-body { font-size:13px; color:#1a1a1a; line-height:1.6; margin-bottom:1rem; font-style:italic; }
.review-name { font-family:'Syne',sans-serif; font-weight:700; font-size:13px; color:#d4a76a; }
.review-role { font-size:11px; color:#1a1a1a; margin-top:.2rem; }
.review-form-card {
    background:#ffffff; border:1px solid #d4a76a;
...
if st.session_state.step >= 2 and st.session_state.ai_data:
    ...
    # ── 3-Month Career Mentorship Card ──
    st.markdown("""
    <div class="card" style="border-color:#ff6b4a66;margin-top:1rem">
        <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.5rem">
            <span style="font-size:1.5rem">🚀</span>
            <div class="card-title" style="margin:0">6-Month Career Coaching - Structured Guidance</div>
        </div>
        <div style="color:var(--muted);font-size:13px;line-height:1.7;margin-bottom:.8rem">
            Not just a tool — structured career guidance for 6 months.<br>
            Monthly strategy calls · Resume updates for every job · LinkedIn profile rewrite · Interview prep · Job search guidance · Structured career guidance.
        </div>
        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:.6rem">
            <span style="background:#ffc10722;color:#cc6600;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">$199 / 6 months</span>
            <span style="background:#ff6b4a22;color:#cc3d1f;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">Structured Coaching</span>
            <span style="background:#ffc10722;color:#cc6600;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">Email + WhatsApp</span>
            <span style="background:#ff6b4a22;color:#cc3d1f;padding:.2rem .7rem;border-radius:20px;font-size:11px;font-weight:600">Limited Spots</span>
        </div>
        <div style="color:var(--muted);font-size:11px;margin-top:.4rem">
            ✅ Resume tailored for every application &nbsp;|&nbsp; ✅ Coaching guidance &nbsp;|&nbsp; ✅ Career strategy support
        </div>
    </div>
    """, unsafe_allow_html=True)

    if st.button("🔔 Notify Me — 6-Month Coaching", key="mentorship_notify", use_container_width=True):
        log_payment_interest("6-Month Coaching $199", st.session_state.email or "")
        st.success("✅ Noted! We will contact you to discuss the coaching program.")
    ...
    <div class="pricing-grid" style="grid-template-columns: repeat(3, 1fr);">
        ...
        <div class="plan hot" style="border-color: #ff6b4a;">
    ...
    <div class="refund-policy">
        ...
        All purchases are final and non-refundable. Once payment is successful and access is activated, 
    ...
    <div class="unlock-box">
        ...
        <div style="color:var(--muted);font-size:12px;margin-bottom:.8rem">
            After payment via UPI or Razorpay, your app shows a Transaction ID — paste it below. Instant unlock, no email needed.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        plan = st.selectbox("Plan", ["Basic $5", "Pro $10", "Premium Yearly $149"], label_visibility="collapsed")
...
if st.session_state.step == 3 and st.session_state.ai_data:
    ...
    if is_pro:
        ...
        tabs = st.tabs(["📝 Cover Letter", "🎯 Interview Kit",
                        "💼 LinkedIn" if is_yearly else "💼 LinkedIn (Premium only)"])
...
"""}],
                ...
                st.info("LinkedIn Profile Rewrite is available in the Premium plan ($20). Upgrade to unlock.")
...
<div class="reviews-grid">
    <div class="review-card">
        ...
        <div class="review-body">"My ATS score jumped from 34% to 78% after tailoring. Got a callback within 3 days. This tool is unreal."</div>
    ...
    <div class="review-card">
        <div class="review-stars">★★★★☆</div>
        <div class="review-body">"The ATS report showed me exactly what keywords I was missing. Tailored my resume in 2 minutes and landed an interview."</div>
        <div class="review-name">Rohit K.</div>
        <div class="review-role">Business Analyst · Hyderabad</div>
    </div>
    <div class="review-card">
        <div class="review-stars">★★★★★</div>
        <div class="review-body">"As a fresher I had no idea how ATS works. This tool explained everything and rewrote my resume perfectly. Got my dream role!"</div>
        <div class="review-name">Sneha T.</div>
        <div class="review-role">Marketing Executive · Pune</div>
    </div>
    <div class="review-card">
        <div class="review-stars">★★★★★</div>
        <div class="review-body">"Used ResumeReflect for my job search. My interview rate jumped from 5% to 32% in just two weeks. Best investment ever!"</div>
        <div class="review-name">James M.</div>
        <div class="review-role">Software Engineer · San Francisco</div>
    </div>
    <div class="review-card">
        <div class="review-stars">★★★★★</div>
        <div class="review-body">"I was hesitant about my resume for international applications. This tool gave me confidence. Got 3 interview offers!"</div>
        <div class="review-name">Sarah J.</div>
        <div class="review-role">Product Manager · New York</div>
    </div>
    <div class="review-card">
        <div class="review-stars">★★★★☆</div>
        <div class="review-body">"Brilliant tool for my job search. Helped me understand what recruiters look for. Already got interview callbacks!"</div>
        <div class="review-name">Emma L.</div>
        <div class="review-role">Data Scientist · London</div>
    </div>
    <div class="review-card">
        <div class="review-stars">★★★★★</div>
        <div class="review-body">"As someone applying internationally, this helped me optimize my resume for global standards. Landed my dream opportunity!"</div>
        <div class="review-name">Michael T.</div>
        <div class="review-role">Software Engineer · Toronto</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="review-form-card">
    <div class="card-title" style="display:flex;align-items:center;gap:0.5rem;">✍️ <span>Share Your Experience</span></div>
    <div style="color:var(--muted);font-size:12px;margin-bottom:1rem;margin-top:0.3rem;">
