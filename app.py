...
if resume_method == "Upload PDF/DOCX":
    uploaded = st.file_uploader("UPLOAD YOUR RESUME", type=["pdf", "docx"], label_visibility="collapsed")
    if uploaded:
        try:
...
# APPEND DETAILED KEYWORDS DISPLAY - AFTER line with "Matched Keywords"
# Replace basic keyword display with professional analysis

EOF_DETAIL
if st.session_state.step >= 2 and st.session_state.ai_data:
    # MATCHED KEYWORDS PROFESSIONAL DISPLAY
    st.markdown("""
    <div style="background: linear-gradient(135deg, rgba(46,204,113,0.1), rgba(26,31,58,0.95)); border: 2px solid #2ecc71; border-radius: 16px; padding: 2.5rem; margin: 2rem 0;">
    <h3 style="color: #2ecc71; margin-bottom: 1.5rem; font-size: 1.4rem;">✅ Matched Keywords Analysis</h3>
    <p style="color: #a0a0a0; margin-bottom: 1.5rem; font-size: 14px;">Your resume already contains these critical keywords. These are strengths to highlight further in interviews.</p>
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 1rem;">
    """ + ''.join([f'<div style="background: rgba(46,204,113,0.2); border: 1px solid #2ecc71; border-radius: 10px; padding: 1rem; text-align: center;"><p style="color: #2ecc71; font-weight: 700; margin: 0;">Keyword {i+1}</p><p style="color: #a0a0a0; font-size: 11px; margin: 0.5rem 0 0 0;">Found in resume</p></div>' for i in range(min(data.get('ats_keywords_found', 0), 12))]) + """
    </div>
    <p style="color: #a0a0a0; margin-top: 1.5rem; font-size: 12px;">💡 Tip: During interviews, elaborate on projects where you used these technologies.</p>
    </div>
    """, unsafe_allow_html=True)
    
    # MISSING KEYWORDS PROFESSIONAL DISPLAY
    st.markdown("""
    <div style="background: linear-gradient(135deg, rgba(231,76,60,0.1), rgba(26,31,58,0.95)); border: 2px solid #e74c3c; border-radius: 16px; padding: 2.5rem; margin: 2rem 0;">
    <h3 style="color: #e74c3c; margin-bottom: 1.5rem; font-size: 1.4rem;">⚠️ Missing Keywords Analysis</h3>
    <p style="color: #a0a0a0; margin-bottom: 1.5rem; font-size: 14px;">These keywords appear in the job description but are missing from your resume. Adding them will improve your ATS score significantly.</p>
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 1rem;">
    """ + ''.join([f'<div style="background: rgba(231,76,60,0.2); border: 1px solid #e74c3c; border-radius: 10px; padding: 1rem; text-align: center;"><p style="color: #e74c3c; font-weight: 700; margin: 0;">Keyword {i+1}</p><p style="color: #a0a0a0; font-size: 11px; margin: 0.5rem 0 0 0;">Add to resume</p></div>' for i in range(min(data.get('ats_keywords_missing', 0), 10))]) + """
    </div>
    <p style="color: #a0a0a0; margin-top: 1.5rem; font-size: 12px;">💡 Tip: Add missing keywords naturally in your experience descriptions and skills section. Our tailored resume includes these.</p>
    </div>
    """, unsafe_allow_html=True)
