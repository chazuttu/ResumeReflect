...
if st.session_state.step >= 2 and st.session_state.ai_data:
    ...
    with col_p:
        if st.button("Pay $10 →", key="pay_pro", use_container_width=True):
            ...
            st.success("🚀 Payments launching soon! We've noted your interest.")
    with col_y:
        if st.button("Pay $50 →", key="pay_yearly", use_container_width=True):
            log_payment_interest("Premium Yearly $50", st.session_state.email or "")
            st.success("🚀 Payments launching soon! We've noted your interest.")
    ...
    with c2:
        plan = st.selectbox("Plan", ["Basic $5", "Pro $10", "Premium Yearly $50"], label_visibility="collapsed")
