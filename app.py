...
# ─────────────────────────────────────────────
    ...
    with col1:
        if st.button("Pro ($5)", use_container_width=True, key="pay_pro"):
            st.markdown(f'<a href="https://buy.razorpay.com/{RAZORPAY_BASIC}" target="_blank" style="text-decoration:none"><button style="width:100%;padding:12px;background:linear-gradient(135deg,#06b6d4,#0f766e);color:white;border:none;border-radius:8px;cursor:pointer;font-weight:700">Pay with Razorpay</button></a>', unsafe_allow_html=True)
    
    with col2:
        if st.button("Premium ($12)", use_container_width=True, key="pay_monthly"):
            st.markdown(f'<a href="https://buy.razorpay.com/{RAZORPAY_PRO}" target="_blank" style="text-decoration:none"><button style="width:100%;padding:12px;background:linear-gradient(135deg,#06b6d4,#0f766e);color:white;border:none;border-radius:8px;cursor:pointer;font-weight:700">Pay with Razorpay</button></a>', unsafe_allow_html=True)
    
    with col3:
        if st.button("Yearly ($50)", use_container_width=True, key="pay_yearly"):
            st.markdown(f'<a href="https://buy.razorpay.com/{RAZORPAY_YEARLY}" target="_blank" style="text-decoration:none"><button style="width:100%;padding:12px;background:linear-gradient(135deg,#06b6d4,#0f766e);color:white;border:none;border-radius:8px;cursor:pointer;font-weight:700">Pay with Razorpay</button></a>', unsafe_allow_html=True)
